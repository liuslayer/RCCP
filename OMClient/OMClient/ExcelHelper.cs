    ...
    public static class ExcelHelper
    {
        ...
        public static void ExportExcels(string fileName, DataGridView myDGV)
        {
            ...
            for (int r = 0; r < myDGV.Rows.Count; r++)
            {
                for (int i = 0; i < myDGV.ColumnCount; i++)
                {
                    worksheet.Cells[r + 2, i + 1] = myDGV.Rows[r].Cells[i].Value == null ? "" : myDGV.Rows[r].Cells[i].Value.ToString();
                    //worksheet.Cells[r + 2, i + 1] = myDGV.Rows[r].Cells[i].Value;//保存GUID时报错：System.Runtime.InteropServices.COMException:“异常来自 HRESULT:0x800A03EC”
