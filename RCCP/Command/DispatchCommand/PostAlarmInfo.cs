...
namespace RCCP.Command
{
    ...
    /// 指挥调度即时消息
    /// </summary>
    public class PostAlarmInfo : CommandBase<CommandDispatchSession, StringRequestInfo>
    {
        public override void ExecuteCommand(CommandDispatchSession session, StringRequestInfo requestInfo)
        {
            string message = CommandUtils.TerminalCommandAssemble(DispatchCommandEnum.PostAlarmInfo.ToString(), requestInfo.Body);
