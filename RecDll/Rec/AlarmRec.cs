...
namespace RecDll
{
    public class AlarmRec
    {
        ...
        /// <returns></returns>
        public static int Rec(string strDeviceID,ref string errorInfo)
        {
            Guid DeviceID;
            if (!Guid.TryParse(strDeviceID, out DeviceID))
            {
                errorInfo="设备ID为"+ strDeviceID;
                return -1;
            }
            ...
            if (!ManualRec.userIDList.ContainsKey(IP)) return -1;
        ...
        {
            Guid DeviceID;
            if (!Guid.TryParse(strDeviceID, out DeviceID)) return false;
            Client.Entities.CameraList device = Class1.cameraList.Find(
                delegate (Client.Entities.CameraList camera)
                {
                    return camera.DeviceID.Equals(DeviceID);
            ...
            if (CHCNetSDK.NET_DVR_StopSaveRealData(m_lRealHandle))
            {
                ...
                if (!result) MessageBox.Show(errorInfo + ",停止录像失败！");
                //移除RecInfo存储信息
